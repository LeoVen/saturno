...
| ID | Task | Status |
|---|---|---|
| INTERLUDE-1-T1 | Navigation shell: persistent left sidebar switching between the 5 existing sections, as in-app reactive state (no router, TR-19) | new |
| INTERLUDE-1-T2 | Design system pass: light-only base stylesheet (palette, type scale, spacing) + shared button/input/card/table styles, replacing each component's ad hoc scoped CSS | new |
| INTERLUDE-1-T3 | `TimeInput` component: fixed 24h HH:MM (hour + 5-minute-step minute selects), replacing every native `<input type="time">` | new |
| INTERLUDE-1-T4 | `WeekGrid` component: reusable weekday × time-row grid, click-to-toggle cells, as a visual layer over the existing data model (no data-model changes) | new |
| INTERLUDE-1-T5 | Apply `WeekGrid` to Segment Time Slot/Break configuration | new |
| INTERLUDE-1-T6 | Apply `WeekGrid` to Teacher Availability configuration | new |
| INTERLUDE-1-T7 | Remove implementation-detail leaks: same-name-Teacher disambiguation without a raw id, decode Assignment's consecutive-periods value into its label everywhere it's shown, audit remaining screens for similar leaks | new |
| INTERLUDE-1-T8 | Regression pass: re-verify E02–E05's Human Verification steps through the reworked UI | new |

...
export function buildTeacherScheduleGrid(
  ...
  const placements = schedule.placements.filter((p) => p.teacherId === teacherId)
  if (placements.length === 0) return null

  const involvedSegments = new Map<string, Segment>()
  for (const p of placements) {
    const segment = classSegment(entities, p.classId)
    if (segment) involvedSegments.set(segment.id, segment)
  }
  if (involvedSegments.size === 0) return null

  const segments = entities.segments.filter((s) => involvedSegments.has(s.id))
  ...
  for (const weekday of WEEKDAYS) {
    const dayPlacements = placements.filter((p) => p.weekday === weekday)
    if (dayPlacements.length === 0) continue

    const resolved = dayPlacements.flatMap((p) => {
      const segment = classSegment(entities, p.classId)
      const slot = segment?.timeSlots.find((t) => t.id === p.timeSlotId)
      return slot ? [{ placement: p, start: slot.start, end: slot.end }] : []
    })
    if (resolved.length === 0) continue

    const dayStart = resolved.reduce(
      (min, r) => (r.start < min ? r.start : min),
      resolved[0]!.start,
    )
    const dayEnd = resolved.reduce((max, r) => (r.end > max ? r.end : max), resolved[0]!.end)
    const byTimeSlotId = new Map(resolved.map((r) => [r.placement.timeSlotId, r]))
    ...
    for (const raw of rawRows) {
      ...
      if (match) {
        const schoolClass = entities.classes.find((c) => c.id === match.placement.classId)
        ...
        const subject =
          entities.subjects.find((s) => s.id === match.placement.subjectId)?.name ?? '?'
        cells.set(key, { kind: 'occupied', classLabel, subject })
